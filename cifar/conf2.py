...
def assert_and_infer_cfg():
    ...
    assert _C.MODEL.ADAPTATION in ["source", "norm", "tent","cotta","vpt"]
...
def load_cfg_fom_args(args,description="Config options."):
    ...
    # log_dest = log_dest.replace('.yaml', f'_SGD_{args.learning_rate}.txt')

    # g_pathmgr.mkdirs(cfg.SAVE_DIR)
    if args.resume is None:
        args.filename = '{}-{}/{}-{}/N_{}/mt_{}/{}-lr_{}-decay_{}/bsz_{}/trial_{}'. \
            format(args.dataset, cfg.MODEL.ARCH,args.method, args.prompt_size, args.N,args.mt_alpha,
                args.optim, args.learning_rate, args.weight_decay, args.batch_size, args.trial)
        args.save_path = 'output3/' + args.filename
        log_dest = log_dest.replace('.yaml', f'.txt')

    else:
        args.save_path = os.path.dirname(args.resume)+'/TTA'
        if args.norm == False:
            log_dest = log_dest.replace('.yaml', f'_{args.optim}_lr{args.learning_rate}_wd{args.weight_decay}_bs{args.batch_size}_nonorm.txt')
        else:
            log_dest = log_dest.replace('.yaml', f'_{args.optim}_lr{args.learning_rate}_wd{args.weight_decay}_bs{args.batch_size}.txt')

    g_pathmgr.mkdirs(args.save_path)
    ...
    logging.basicConfig(
        ...
        datefmt="%y/%m/%d %H:%M:%S",
        handlers=[
            logging.FileHandler(os.path.join(args.save_path, cfg.LOG_DEST)),
